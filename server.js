...
try {
  ...
  if (process.env.SUPABASE_URL && process.env.SUPABASE_ANON_KEY) {
    ...
    console.warn(
      "SUPABASE_URL or SUPABASE_ANON_KEY not found in environment variables."
    );
    ...
    console.warn(
      "2. Add SUPABASE_ANON_KEY=your_supabase_anon_key to your .env file"
    );
  ...
  if (process.env.SEMRUSH_API_KEY) {
    semrushConnector = new SemrushConnector(
    ...
    const health = {
      ...
      services: {
        ...
        semrush: {
          ...
          working: semrushConnector
            ? await semrushConnector.testConnection()
            : false,
        },
      },
    };
...
app.post("/api/generate-ad-copy", async (req, res) => {
  try {
    ...
    console.log(
      "Generating ad copy for client:",
      req.body.clientInfo?.name || "Unknown"
    );

    const { clientInfo, campaignContext, adGroupContext, savedKeywords } =
      req.body;
    ...
    if (savedKeywords && savedKeywords.length > 0) {
      ...
      keywordData = {
        keywords: {
          ...
          lowVolume: [],
          lowCompetition: formattedKeywords.filter(
            (kw) => kw.matchType === "exact"
          ),
          all: formattedKeywords,
        },
        analysis: {
          coreTopics: [adGroupContext?.name || "General"],
          industryTerms: campaignContext?.isUnitType
            ? ["real estate", "property", "homes"]
            : [clientInfo?.industry || "business"],
          seedKeywords: formattedKeywords.slice(0, 5).map((kw) => kw.keyword),
          targetingInsights: `Keywords saved for ${
            adGroupContext?.name || "this ad group"
          }`,
        },
        recommendations: {
          primaryKeywords: formattedKeywords.slice(0, 5),
          longTail: formattedKeywords.filter(
            (kw) => kw.keyword.split(" ").length >= 3
          ),
          branded: [],
        },
      };
      ...
      return res.status(400).json({
        ...
        error:
          "Please generate and save keywords for this ad group before creating ad copy. Go to the Keywords tab, generate keywords, and save them first.",
      });
    }
...
app.post("/api/generate-keywords", async (req, res) => {
  try {
    ...
    console.log(
      "Generating keywords for client:",
      req.body.clientInfo?.clientName || "Unknown"
    );
    ...
    if (!semrushConnector) {
      ...
      return res.json({
        ...
        data: {
          keywords: [],
          message:
            "Configure Semrush API key to generate keyword recommendations",
        },
      });
    }
    ...
    if (!req.body.adGroupContext || !req.body.adGroupContext.name) {
      throw new AppError(
        ...
        "Ad group name is required for keyword generation"
      );
    }
...
app.post("/api/migrate-data", async (req, res) => {
  try {
    ...
    const result = await databaseService.migrateLocalStorageData(
      clientInfo,
      campaignsData
    );
...
app.get("/api/campaigns/:campaignId", async (req, res) => {
  try {
    ...
    const campaign = await databaseService.getCampaignWithFullStructure(
      campaignId
    );
...
>>>>>>> 7d9cc8e327886a04faff6aa6a5c3de962f135e85
    }
...
// Delete Ad Group and all associated keywords and ads
app.delete("/api/ad-groups/:adGroupId", async (req, res) => {
  try {
    ...
    // Delete keywords for this ad group
    await databaseService.deleteKeywordsByAdGroup(adGroupId);
    // Delete ads for this ad group
    const ads = await databaseService.getAdsByAdGroup(adGroupId);
    for (const ad of ads) {
      await databaseService.deleteAd(ad.id);
    }
    // Delete the ad group itself
    await databaseService.deleteAdGroup(adGroupId);
    res.json({
      success: true,
      message:
        "Ad group and all associated keywords and ads deleted successfully",
    });
  } catch (error) {
    console.error("Error deleting ad group:", error);
    res.status(500).json({
      success: false,
      message: "Error deleting ad group",
      error: error.message,
    });
  }
});
