...
class DatabaseService {
  ...
  async deleteAd(adId) {
    const { error } = await this.supabase.from("ads").delete().eq("id", adId);
  ...
  async migrateLocalStorageData(clientInfo, campaignsData) {
    ...
    try {
      ...
      // 2. Create campaigns
      for (const [localCampaignId, localCampaign] of Object.entries(
        campaignsData
      )) {
        const campaign = await this.createCampaign({
          ...
          budget: localCampaign.budget
            ? parseFloat(localCampaign.budget)
            : null,
        ...
        if (localCampaign.adGroups) {
          for (const [localAdGroupId, localAdGroup] of Object.entries(
            localCampaign.adGroups
          )) {
            ...
            // 4. Create keywords
            if (
              localCampaign.keywords &&
              localCampaign.keywords[localAdGroupId]
            ) {
              const keywordData = localCampaign.keywords[localAdGroupId].map(
                (kw) => ({
