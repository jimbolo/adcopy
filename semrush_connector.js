...
class SemrushConnector {
  ...
  async analyzeClientInfo(
    ...
    try {
      ...
      const isUnitTypeCampaign =
        (!clientInfo || Object.keys(clientInfo).length === 0) &&
        !campaignContext &&
        adGroupContext &&
        adGroupContext.name;

      // Check if this is a General Search campaign (isUnitType=false) with Location ad group
      const isGeneralSearchCampaign =
        campaignContext &&
      ...
      if (isGeneralSearchCampaign) {
        ...
        console.log(
          "General Search campaign detected. Generating location-based keywords."
        );
...
Client Information:
- Client Name: ${
              clientInfo.clientName || clientInfo.name || "Property Management"
            }
...
Generate keywords that would attract searchers looking for properties in the specified location with nearby amenities, schools, and public works.`,
        ...
        const analysis = await this.openaiConnector.generateResponse(
          locationPrompt,
          "gpt-4o"
        );
        ...
        console.log(
          "General Search campaign analysis completed:",
          analysisData
        );
        return analysisData;
      }
      ...
      if (isUnitTypeCampaign) {
        ...
        console.log(
          "Unit type campaign detected. Using ad group name only:",
          adGroupName
        );
...
- Ad Group Target Audience: ${
          adGroupContext.targetAudience || "Not specified"
        }`;
      }
...
${
  campaignContext
    ? "IMPORTANT: Pay special attention to the campaign context and tailor keywords to support the campaign objective."
    : ""
}
${
  adGroupContext
    ? "IMPORTANT: Focus keywords specifically around the ad group theme and target audience for maximum relevance."
    : ""
}
...
Focus on extracting terms that would be valuable for finding relevant keywords in Semrush that align with the specific campaign strategy and ad group focus.`,
      ...
      throw new Error(
        "Failed to analyze client information for keyword research"
      );
    }
  }
    ...
    for (const seedKeyword of seedKeywords) {
      promises.push(
        this.searchKeywords(seedKeyword, database, limitPerKeyword)
          .then((results) => {
            console.log(
              `✅ Keywords found for "${seedKeyword}": ${results.length}`
            );
            return results;
          })
          .catch((error) => {
            console.warn(
              `❌ Failed to fetch keywords for "${seedKeyword}":`,
              error.message
            );
  ...
  async generateKeywordRecommendations(
    ...
    try {
      ...
      const analysis = await this.analyzeClientInfo(
        clientInfo,
        campaignContext,
        adGroupContext
      );
      ...
      console.log(
        "Keyword suggestions from Semrush:",
      ...
      const result = {
        ...
        keywords: {
          ...
          mediumVolume: keywordSuggestions.filter(
            (kw) => kw.searchVolume >= 100 && kw.searchVolume <= 500
          ),
          lowVolume: keywordSuggestions.filter(
            (kw) => kw.searchVolume < 100 && kw.searchVolume > 0
          ),
          lowCompetition: keywordSuggestions.filter(
            (kw) => kw.competition < 0.3
          ),
          all: keywordSuggestions.slice(0, 200),
        },
        recommendations: {
          primaryKeywords: keywordSuggestions.slice(0, 10),
          longTail: keywordSuggestions
            .filter((kw) => kw.keyword.split(" ").length >= 3)
            .slice(0, 15),
          branded: keywordSuggestions.filter(
            (kw) =>
              clientInfo.clientName &&
              kw.keyword
                .toLowerCase()
                .includes(clientInfo.clientName.toLowerCase())
          ),
        },
      };
      ...
      if (campaignContext || adGroupContext) {
        result.contextualInsights = {
          campaignAlignment:
            analysis.campaignAlignment || "No campaign context provided",
          adGroupAlignment:
            analysis.adGroupAlignment || "No ad group context provided",
          recommendedFocus: this.generateContextualRecommendations(
            analysis,
            campaignContext,
            adGroupContext
          ),
  ...
  generateContextualRecommendations(analysis, campaignContext, adGroupContext) {
    ...
    if (campaignContext) {
      recommendations.push(
        `Campaign "${campaignContext.name}" focus: Prioritize keywords that align with the campaign objective.`
      );
    }

    if (adGroupContext) {
      recommendations.push(
        `Ad Group "${adGroupContext.name}" theme: Focus on keywords that match the ad group's specific targeting.`
      );
    }
