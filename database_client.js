...
class DatabaseClient {
  ...
  async getCampaignsByClient(clientId = null) {
    ...
    try {
      const response = await fetch(
        `${this.apiBaseUrl}/api/clients/${targetClientId}/campaigns`
      );
      if (!response.ok) {
  ...
  async getCurrentClient() {
    ...
    try {
      const response = await fetch(
        `${this.apiBaseUrl}/api/clients/${this.currentClientId}`
      );
      if (!response.ok) {
  ...
  async getCampaign(campaignId) {
    try {
      const response = await fetch(
        `${this.apiBaseUrl}/api/campaigns/${campaignId}`
      );
      if (!response.ok) {
...
>>>>>>> 7d9cc8e327886a04faff6aa6a5c3de962f135e85
  ...
  // This could return null or a string, not always a boolean.
  //
  // Updated implementation:
  // Always returns a boolean (true/false) for reliable conditional logic in the app.
  // Reason: Ensures that checks like if (databaseClient.isUsingSupabase()) work as expected.
  isUsingSupabase() {
    return !!(this.isSupabaseEnabled && this.currentClientId);
  }
